...
public class PersonController {
    ...
    public Person getPerson(@PathVariable int id, @RequestParam(defaultValue = "1") int delay)
            throws InterruptedException {
        Thread.sleep(delay * 1000);
        if (personList.stream().filter((person) -> person.getId() == id)
                .findFirst()
                .isEmpty()) {
            return new Person(0, "ERROR - Not Found");
        } else{
